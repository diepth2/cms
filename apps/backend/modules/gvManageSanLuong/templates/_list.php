<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_gv_manage_san_luong" style="margin-top: 5px !important;">
      <thead>
          <tr>
...
<!--                        </th>-->
          ...
          <?php include_partial('gvManageSanLuong/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End diepth2   -->
        <?php foreach ($results as $i => $log_payment): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $log_payment->getPrimaryKey() ?>}" rel="<?php echo $log_payment->getPrimaryKey() ?>">

                            <?php #include_partial('gvManageSanLuong/list_td_batch_actions', array('log_payment' => $log_payment, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('gvManageSanLuong/list_td_tabular', array('log_payment' => $log_payment, 'orderNo' => $orderNo)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="5">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('gvManageSanLuong/pagination', array('pager' => $pager)) ?></div>
