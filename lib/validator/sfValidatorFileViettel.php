...
class sfValidatorFileViettel extends sfValidatorFile
{
  ...
  protected function doClean($value)
  {
    ...
    if ($this->hasOption('max_size') && $this->getOption('max_size') < (int)$value['size']) {
     ...
     $type = $this->getMimeType((string)$value['tmp_name'], (string)$value['type']);
    ...
    if ($this->hasOption('mime_types')) {
     ...
     //ngoctv: Kiem tra content file co phai la image khong
      $im = false;
      if ($this->hasOption('is_image')) {
            $im=$this->getOption('is_image');
        }
      if($im){
           $img = imagecreatefromstring(file_get_contents($value['tmp_name']));
      }
      //  kiem tra file up len co phai la excel khong, neu dung thi doc file
        //doc file, neu khong phai thi thong bao loi
      if (!in_array($mimeType, array_map('strtolower', $mimeTypes)) || !in_array($type, array_map('strtolower', $mimeTypes))|| (isset($img) &&!$img)) {
    ...
    protected function canRead($pFilename)
    {
        ...
        try{
            ...
            return $objWorksheet? true: false;
