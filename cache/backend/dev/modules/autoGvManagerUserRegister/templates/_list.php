<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_gv_manager_user_register" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('gvManagerUserRegister/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End diepth2   -->
        <?php foreach ($results as $i => $user): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          ...
          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $user->getPrimaryKey() ?>}" rel="<?php echo $user->getPrimaryKey() ?>">
                            ...
                            <?php include_partial('gvManagerUserRegister/list_td_batch_actions', array('user' => $user, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('gvManagerUserRegister/list_td_tabular', array('user' => $user, 'orderNo' => $orderNo)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="8">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('gvManagerUserRegister/pagination', array('pager' => $pager)) ?></div>
