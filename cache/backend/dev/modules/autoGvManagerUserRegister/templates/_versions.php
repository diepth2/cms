...
<div class="clearfix" style="margin-bottom:18px">
    <?php $all_versions = Doctrine_Query::create()->from(get_class($user->getRawValue()) . 'Version fv')->where('fv.id = ?', $user->getId())->andWhere('fv.version <> ?', $user->get('version'))->orderBy('fv.version DESC')->execute(); ?>
