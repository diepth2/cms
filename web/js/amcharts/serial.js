AmCharts.AmSerialChart = AmCharts.Class({inherits: AmCharts.AmRectangularChart, construct: function (a) {
    ...
    if (!this.wheelBusy) {
        ...
        0 > a ? d ? (l = this.endTime - this.startTime, d = this.startTime + c * e, e = this.endTime + f * e, 0 < f && 0 < c && e >= h && (e = h, d = h - l), this.zoomToDates(new Date(d), new Date(e))) : (0 < f && 0 < c && this.end >= l - 1 && (c = f = 0), d = this.start +
            c, e = this.end + f, this.zoomToIndexes(d, e)) : d ? (l = this.endTime - this.startTime, d = this.startTime - c * e, e = this.endTime - f * e, 0 < f && 0 < c && d <= g && (d = g, e = g + l), this.zoomToDates(new Date(d), new Date(e))) : (0 < f && 0 < c && 1 > this.start && (c = f = 0), d = this.start - c, e = this.end - f, this.zoomToIndexes(d, e))
    }
    ...
    0 < c.length && (this.firstTime = this.getStartTime(c[0].time), this.lastTime = this.getEndTime(c[c.length -
        1].time))
    ...
    updateScrollbars: function () {
        ...
        if (0 < d && b - a < d) {
            var e =
                Math.round(a + (b - a) / 2), d = Math.round(d / 2);
            a = e - d;
            b = e + d
        }
        ...
        for (b = 0; b < a.length; b++) {
            ...
            AmCharts.isString(c.fillToGraph) && (c.fillToGraph =
                this.getGraphById(c.fillToGraph))
        }
    ...
    compareDepth: function (a, b) {
        ...
        for (b = 0; b < a.length; b++) {
            ...
            a[b] = c;
            c.chart = this;
            AmCharts.isString(c.valueAxis) && (c.valueAxis =
                this.getValueAxisById(c.valueAxis));
        ...
        var a = 0, b = this.valueAxes.length, c = this.graphs.length, d, e, f = !1,
            l, h;
        for (h = 0; h < b; h++) {
            e = this.valueAxes[h];
            var g = e.stackType;
            ...
            if ("none" == g || "3d" == g) {
                f = !1;
                for (l = 0; l < c; l++)d = this.graphs[l], d.valueAxis == e && "column" == d.type && (d.clustered ? (d.tcc = 1, d.newStack && (a = 0), d.hidden || (d.columnIndex = a, a++)) : d.hidden || (f = !0, d.tcc = 1, d.columnIndex = 0));
                f &&
                    0 == a && (a = 1)
            }
            if ("3d" == g) {
    ...
    formatPeriodString: function (a, b) {
        if (b) {
            ...
            for (var h = 0; h < c.length; h++) {
                for (var g = c[h], k = 0, m = 0, p, v, r, t, n, w = 0, q = 0, x, u, z, y, B, D = this.start; D <= this.end; D++) {
                    ...
                    if (s && (s = s.axes[e.id].graphs[b.id])) {
                        if (s.values) {
                            var A = s.values[g];
                            if (this.rotate) {
                                if (0 > s.x || s.x > s.graph.height)A = NaN
                            } else if (0 > s.x || s.x > s.graph.width)A = NaN;
                            if (!isNaN(A)) {
                                isNaN(p) &&
                                (p = A);
                                v = A;
                ...
                w = {open: x, close: u, high: y, low: z, average: B, sum: w, count: q};
                a = AmCharts.formatValue(a, {open: p, close: v, high: t, low: r, average: n, sum: k, count: m},
...
AmCharts.Cuboid = AmCharts.Class({construct: function (a, b, c, d, e, f, l, h, g, k, m, p, v, r, t, n) {
    this.set = a.set();
    this.container = a;
    this.h = Math.round(c);
    this.w = Math.round(b);
    ...
    var b = this.container, c = this.w, d =
        this.h, e = this.dx, f = this.dy, l = this.colors, h = this.alpha, g = this.bwidth, k = this.bcolor, m = this.balpha, p = this.gradientRotation, v = this.cornerRadius, r = this.dashLength, t = this.pattern, n = this.topRadius, w = l, q = l;
    "object" == typeof l && (w = l[0], q = l[l.length - 1]);
    var x, u, z, y, B, D, s, A, H, M = h;
    t && (h = 0);
    var C, E, F, G, I = this.rotate;
    if (0 < Math.abs(e) || 0 < Math.abs(f))if (isNaN(n))s = q, q = AmCharts.adjustLuminosity(w, -.2), q = AmCharts.adjustLuminosity(w, -.2), x = AmCharts.polygon(b, [0, e, c + e, c, 0], [0, f, f, 0, 0], q, h, 1, k, 0, p), 0 < m && (H = AmCharts.line(b,
        ...
        .1 < F && .1 < F && (s = AmCharts.circle(b, F, AmCharts.adjustLuminosity(w, .5), h, g, k, m, !1, G), s.translate(K, L))
    }
    h = M;
    1 > Math.abs(d) && (d = 0);
    1 > Math.abs(c) && (c = 0);
    ...
    c = isNaN(n) ? 0 > d ? [x, H, u, z, y, B, D, s, A, b] : [s, A, u, z, y, B, x, H, D, b] : I ? 0 < c ? [x, b, s] : [s, b, x] : 0 > d ? [x, b, s] : [s, b, x];
...
AmCharts.CategoryAxis = AmCharts.Class({inherits: AmCharts.AxisBase, construct: function (a) {
    ...
    if (AmCharts.ifArray(a)) {
        ...
        var v, r, t, n, w, q;
        v = this.rotate;
        var x = this.firstDayOfWeek, u = this.boldPeriodBeginning, a = AmCharts.resetDateToMin(new Date(a[a.length -
        ...
        if (this.parseDates && !this.equalSpacing) {
            ...
            var l = Math.ceil(this.timeDifference / r) + 5, D = m = AmCharts.resetDateToMin(new Date(this.startTime - r), e,
                v, x).getTime();
            t == e && 1 == v && this.centerLabelOnFullPeriod && (w = r * this.stepWidth);
            ...
            if (0 < this.gridCountR)for (b = d; b <= l; b++) {
                ...
                q = !1;
                this.nextPeriod[t] && (q = this.checkPeriodChange(this.nextPeriod[t], 1, p, m, t));
                z = !1;
                q && this.markPeriodChange ? (q = this.dateFormatsObject[this.nextPeriod[t]], this.twoLineMode && (q = this.dateFormatsObject[t] + "\n" + q, q = AmCharts.fixBrakes(q)), z = !0) : q = this.dateFormatsObject[t];
                u || (z = !1);
            ...
            if (this.cellWidth = this.getStepWidth(b), b < l && (l = b), f += this.start, this.stepWidth = this.getStepWidth(b), 0 < l)for (u = Math.floor(b / l), y = this.chooseMinorFrequency(u),
                ...
                n = !1;
                0 <= b && b < this.data.length ? (t = this.data[b], m = t.category, n = t.forceShow) : m = "";
                if (q && !isNaN(y))if (b / y == Math.round(b / y) || n)b / u == Math.round(b / u) || n || (this.gridAlpha = this.minorGridAlpha, m = void 0); else continue; else if (b / u != Math.round(b / u) && !n)continue;
                ...
                D = this.cellWidth;
                v && (D = NaN);
                this.labelFunction && t && (m = this.labelFunction(m, t, this));
            ...
            if (0 < this.gridCountR)for (b = k; b < w; b++)if (p = this.data[b].time, this.checkPeriodChange(e,
                ...
                this.nextPeriod[t] && (q = this.checkPeriodChange(this.nextPeriod[t], 1, p, m, t));
                z = !1;
                q && this.markPeriodChange ? (q = this.dateFormatsObject[this.nextPeriod[t]], z = !0) : q = this.dateFormatsObject[t];
                m = AmCharts.formatDate(new Date(p), q, c);
                if (b == d && !h || b == l && !g)m = " ";
                B ? B = !1 : (u || (z = !1), k - this.previousPos > this.safeDistance * Math.cos(this.labelRotation * Math.PI / 180) && (this.labelFunction && (m = this.labelFunction(m, new Date(p), this, e, v, n)), this.boldLabels && (z = !0), n = new this.axisItemRenderer(this,
