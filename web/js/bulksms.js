...
//    $("#vt_bulksms_contact_birthday").datetimepicker({
//        changeMonth: true,
//        changeYear: true,
//        dateFormat: 'dd-mm-yy',
//        showButtonPanel: true,
...
//    });
    ...
    $('#vt_bulksms_content_content').on('input paste',function(){
        ...
        var numsms =  Math.floor(len/160) + 1;
        if(len==0) numsms = 0;
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if(data.status != 1){
                        ...
                        var len = 200;
                        content_convert= data.content_convert;
                        $("#brandname-modal").text(brandname);
                        $("#schedule_at-modal").text(schedule_at);
                        $("#content-modal").text(content_convert);
                        $("#total-contact").text(data.total_contact);
                        $("#total-costs").text(data.sms_fee*data.total_contact);
    ...
    $("#btn-confirmSendSMS").click(function(){
        ...
        var brandname_id = $("#vt_bulksms_content_brandname_id").val();
        var content = $("#vt_bulksms_content_content").val();
        var content_type_id = $("#vt_bulksms_content_type_id").val();
        var schedule_at = $("#vt_bulksms_content_schedule_at").val();
        //
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if(data.status == 1){
                    ...
                    $("#publisher_money").text(data.publisher_money + "VNĐ");
            ...
            success: function (result) {
                ...
                if(data.error > 0){
                    $("#list-brandName").append('<tr>' +
                        ...
                        '<td class="vtt-center">' +
                        '<a href="javascript:void(0);" title="Sửa"><span class="glyphicon glyphicon-pencil update-brandName-info" id="'+data.brand_id+'"></span></a>' +
                        '</td>' +
                        '</tr>"');
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if(error == -3){
                    ...
                    $("#update-ajax-error").html(data.message);
                }
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if(returnResult.status ==1){
                    ...
                    $("#tbody-contact-group tr #contact-name-" + ids[1]).text(name);
                    $("#tbody-contact-group tr #contact-phone-" + ids[1]).text(phone);
                } else if (returnResult.status == -2){
                    location.reload();
    ...
    $("#add-group-popup").click(function() {
        var groupName = trim($("#groupName").val().trim());
        var groupNote = trim($("#groupNote").val().trim());
        var token = $("#token-bulksms").val();
        $.ajax({
            type:"GET",
            ...
            success:function (result) {
                ...
                if(returnResult.status ==-4){
                    ...
                    $("#list-bulk-group").prepend('<tr id="delete-item-' + returnResult.group_id+ '">' +
                                ...
                                '<td class="vtt-center" id="group-name-'+ returnResult.group_id+'">' + groupName + '</td>' +
                                '<td class="vtt-center">0</td>' +
                                '<td class="vtt-center">' +
                                    '<a href="javascript:void(0);" title="Sửa"><span class="glyphicon glyphicon-pencil update-group-info" id="'+returnResult.group_id+ '"></span></a>' +
                    ...
                    $('#vt_bulksms_contact_group_info').prepend('<option value="' + returnResult.group_id+ '">'+groupName +'</option>');
                    $('#filter-contact-group').append('<option value="' + returnResult.group_id+ '">'+groupName +'</option>');
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (returnResult.status == 1){
                    ...
                    if(document.getElementById('num-contact-group').getAttribute('value') <= 0){
